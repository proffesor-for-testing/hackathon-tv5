...
//!
//! Normalizes HBO Max (now Max) content with proper genre mapping,
...
/// HBO Max (Max) normalizer
pub struct HboMaxNormalizer {
    client: Client,
    api_key: String,
    base_url: String,
}

impl HboMaxNormalizer {
    /// Create a new HBO Max normalizer
    ...
    /// Map HBO Max genres to canonical taxonomy
    fn map_hbo_genre(&self, hbo_genre: &str) -> Vec<String> {
        match hbo_genre.to_lowercase().as_str() {
            // HBO-specific genres
            "hbo originals" | "hbo original" => vec!["Drama".to_string()],
            "max originals" | "max original" => vec!["Drama".to_string()],
            "prestige drama" => vec!["Drama".to_string()],
            "limited series" => vec!["Drama".to_string()],
            "hbo documentary" | "hbo documentaries" => vec!["Documentary".to_string()],
    ...
    fn get_subscription_tier(&self, data: &serde_json::Value) -> String {
        ...
        if let Some(streaming_info) = data.get("streamingInfo") {
            if let Some(hbo) = streaming_info.get("hbo") {
                if let Some(us) = hbo.get("us") {
    ...
    /// Check if content is a Max Original
    fn is_max_original(&self, data: &serde_json::Value) -> bool {
        if let Some(genres) = extract_array(data, "genres") {
            for genre in genres {
                if let Some(g) = genre.as_str() {
                    let lower = g.to_lowercase();
                    if lower.contains("max original") || lower.contains("hbo original") {
                        return true;
                    }
                }
            }
...
#[async_trait]
impl PlatformNormalizer for HboMaxNormalizer {
    ...
    async fn fetch_catalog_delta(
        ...
        let url = format!(
            "{}/changes?country={}&since={}&service=hbo&show_type=all",
            self.base_url,
            region,
            since.format("%Y-%m-%d")
        );
        ...
        let changes = data
            ...
            .ok_or_else(|| {
                IngestionError::NormalizationFailed(
                    "No changes array in HBO Max response".to_string(),
                )
            })?;
    ...
    fn normalize(&self, raw: RawContent) -> Result<CanonicalContent> {
        ...
        if let Some(genre_array) = extract_array(data, "genres") {
            for g in genre_array {
                if let Some(genre_str) = g.as_str() {
                    genres.extend(self.map_hbo_genre(genre_str));
                }
            }
        }
        // Deduplicate genres
        ...
        // Add Max Original badge if applicable
        let is_original = self.is_max_original(data);
        ...
        // Add Max Original flag as metadata
        if is_original {
            external_ids.insert("max_original".to_string(), "true".to_string());
        }
        external_ids.insert("subscription_tier".to_string(), subscription_tier);

        Ok(CanonicalContent {
            platform_content_id: raw.id,
            platform_id: "hbo_max".to_string(),
            entity_id: None,
            title,
    ...
    fn generate_deep_link(&self, content_id: &str) -> DeepLinkResult {
        DeepLinkResult {
            mobile_url: Some(format!("hbomax://deeplink?contentId={}", content_id)),
            web_url: format!("https://play.max.com/video/watch/{}", content_id),
            tv_url: Some(format!("hbomax://deeplink?contentId={}", content_id)),
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_hbo_genre_mapping() {
        let normalizer = HboMaxNormalizer::new("test_key".to_string());

        assert_eq!(normalizer.map_hbo_genre("hbo originals"), vec!["Drama"]);
        assert_eq!(normalizer.map_hbo_genre("max originals"), vec!["Drama"]);
        assert_eq!(normalizer.map_hbo_genre("prestige drama"), vec!["Drama"]);
        assert_eq!(
            normalizer.map_hbo_genre("true crime"),
            vec!["Crime", "Documentary"]
        );
        assert_eq!(normalizer.map_hbo_genre("comedy"), vec!["Comedy"]);
        assert_eq!(
            normalizer.map_hbo_genre("dc universe"),
    ...
    fn test_deep_link_generation() {
        let normalizer = HboMaxNormalizer::new("test_key".to_string());
        let deep_link = normalizer.generate_deep_link("urn:hbo:episode:GXssdfg");

        assert!(deep_link.mobile_url.unwrap().contains("hbomax://"));
        assert!(deep_link.web_url.contains("play.max.com"));
        assert!(deep_link.tv_url.unwrap().contains("hbomax://"));
    }
    ...
    #[test]
    fn test_max_original_detection() {
        let normalizer = HboMaxNormalizer::new("test_key".to_string());

        let original_data = serde_json::json!({
            "genres": ["Max Original", "Drama"]
        });
        assert!(normalizer.is_max_original(&original_data));

        let non_original_data = serde_json::json!({
            "genres": ["Drama", "Thriller"]
        });
        assert!(!normalizer.is_max_original(&non_original_data));
    }
}
